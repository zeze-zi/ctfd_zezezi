...
from .models import ZezeziContainer, DynamicDockerChallenge

class DynamicValueDockerChallenge(BaseChallenge):
    id = "zezezi_dynamic_docker"  # Unique identifier used to register challenges
    name = "zezezi_dynamic_docker"  # Name of a challenge type
    ...
    def attempt(cls, challenge, request):
        ...
        if len(flags) > 0:
            ...
            q = q.filter(ZezeziContainer.user_id == user_id)
            q = q.filter(ZezeziContainer.challenge_id == challenge.id)
            records = q.all()
            if len(records) == 0:
    ...
    def delete(cls, challenge):
        for container in ZezeziContainer.query.filter_by(
