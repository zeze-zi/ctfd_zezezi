...
async function delete_container(user_id, challenge_id) {
    let response = await CTFd.fetch("/api/v1/plugins/zezezi/admin/container?user_id=" + user_id + "&challenge_id=" + challenge_id, {
        method: "DELETE",
...
async function renew_container(user_id, challenge_id) {
    let response = await CTFd.fetch(
        "/api/v1/plugins/zezezi/admin/container?user_id=" + user_id + "&challenge_id=" + challenge_id, {
        method: "PATCH",
        credentials: "same-origin",
        headers: {
            Accept: "application/json",
...
$('#containers-delete-button').click(function (e) {
    ...
    CTFd.ui.ezq.ezQuery({
        ...
        body: `是否确实要删除所选内容 ${containers.length} 容器?`,
        success: async function () {
